...
const { EmbedBuilder } = require('../utils/v2EmbedBuilder');
...
async function promptUserToPickAnime(interaction, title, candidates) {
  ...
  await interaction.editReply({
    flags: 32768, components: [pickerEmbed, ...initial.components],
  });
    ...
    collector.on('collect', async (componentInteraction) => {
      ...
      if (componentInteraction.isButton()) {
        ...
        await componentInteraction.update({
          flags: 32768, components: [buildAnimePickerEmbed(title, candidates, page), ...built.components],
        });
        return;
      }
